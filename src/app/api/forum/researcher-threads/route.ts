...
export async function GET(req: NextRequest) {
  try {
    ...
    // For researcher-only threads, we might use a special category or tag
    // For now, let's assume they're in a special "researcher-discussion" category
    const query = { category: 'researcher-discussion' };
    
    const posts = await ForumPostModel.find(query)
      .sort({ createdAt: -1 })
      .limit(limit)
      .skip(offset)
      .populate('authorId', 'firstName lastName institution');
