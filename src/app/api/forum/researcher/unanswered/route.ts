...
export async function GET(req: NextRequest) {
  try {
    ...
    // Find unanswered posts in researcher's expertise areas
    const query = {
    ...
    // Add reply counts to posts and filter for truly unanswered (0 replies)
    const unansweredPosts = posts
      .map(post => ({
        ...post.toObject(),
